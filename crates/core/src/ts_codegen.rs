...
pub(crate) fn render_typescript(
    ...
    // like the built-in events/requests above — because the exporter runs on a
    // bare `App` (`register_bindings` only) while `ReactUiPlugin` always
    // registers the built-ins at runtime. Unlike the reserved event names, a
    // custom filter claiming a built-in name *wins*, mirroring the registry's
    // warn-and-replace runtime semantics (`register_entry`).
    let mut builtin_filters = FilterRegistry::default();
    builtin_filters.register_builtins();
    let mut filter_rows: Vec<(&str, String)> = filters
        .entries
        .iter()
        .map(|(name, reg)| {
            (reg.ts_collect)(&mut collector);
    ...
    for (name, reg) in &builtin_filters.entries {
        if !filters.entries.contains_key(name) {
            (reg.ts_collect)(&mut collector);
            filter_rows.push((*name, (reg.ts_name)()));
        }
    }
    filter_rows.sort();
    ...
    // The filter registry: augments the (empty) `BevyFilters` interface the
    // `bevy-react` package exports, turning the `filter` style field's
    // `FilterUse` mapped union into one typed entry per registered filter. The
    // specifier must be the package name app code imports — that is the module
    // whose `BevyFilters` declaration merging targets.
    out.push_str(
        "/** Every registered filter name and its params type. Augments the empty\n\
         \x20*  `BevyFilters` registry interface in the `bevy-react` package, so the\n\
         \x20*  `filter` style field types each name's params. */\n\
...
mod tests {
    ...
    /// eight built-ins and any customs merged and sorted by wire name.
    ...
    /// eight built-ins are still seeded — they are always available at runtime

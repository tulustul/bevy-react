...
pub struct Style {
    ...
    /// `layoutRounding`: whether bevy rounds this node's laid-out rect to
    /// whole physical pixels (`LayoutConfig::use_rounding`). Absent = inherit
    /// from the nearest ancestor that sets it (the root default is `true`);
    /// inheritance is downward only and restarts at every detached root
    /// (`<surface>`, `<root>`), so set it on the PARENT that lays out the
    /// animated node and its neighbours. `false` lays that subtree out at
    /// fractional px — the fix for the 1px hops of any real-layout size
    /// animation, at the price of soft edges and blurred text wherever
    /// content rests on a half pixel. `no_overlay`: a rounding switch has no
    /// per-variant meaning (a hover flip would re-lay the subtree out).
    #[serde(default)]
    pub layout_rounding: Option<bool>,
    #[serde(default)]
...
pub mod style_groups {
    ...
    /// `LayoutConfig` (reads `layout_rounding`) — bevy's per-subtree pixel
    /// rounding switch. A component write only: bevy picks the rounded or
...
macro_rules! with_style_fields {
    ($cb:ident) => {
        $cb! {
            ...
            (layout_rounding, "layoutRounding", (LAYOUT_ROUNDING), no_overlay),

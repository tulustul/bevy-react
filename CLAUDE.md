...
- The `canvas`, `svg`, `animations`, and `filters` modules each **own their wire types** (`DrawCmd`, `ShapeAttrs`/`ViewBox`/`PathData`, `AnimatedBindings`, `FilterChain`/`FilterUse`); `core`'s `protocol::Props` references them by module path (and re-exports `DrawCmd` as `protocol::DrawCmd`). `filters` additionally owns the `FilterRegistry` + built-ins, param packing (`ParamSlot`, no-straddle rule), and the resolve/interpolation primitives (`resolve_filter_chains`, `lerp_packed_params`). The dependency direction is module-level convention: the bridge machinery (`protocol`/`reconcile`/`ui_map`) reaches into `crate::canvas`/`crate::animations`/`crate::filters` — never the reverse — so adding a draw command, animation driver, or filter is a change in that module, not the bridge. (`layer::render` consumes `filters`' resolved chains; the filter resolve system reads `layer`'s promotion markers — that pair is deliberately coupled.)

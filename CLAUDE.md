...
- The `canvas` and `animations` modules each **own their wire types** (`DrawCmd`, `AnimatedBindings`); `core`'s `protocol::Props` references them by module path (and re-exports `DrawCmd` as `protocol::DrawCmd`). The dependency direction is module-level convention: `protocol`/`reconcile`/`ui_map` reach into `crate::canvas`/`crate::animations` — never the reverse — so adding a draw command or animation driver is a change in that module, not the bridge machinery.
